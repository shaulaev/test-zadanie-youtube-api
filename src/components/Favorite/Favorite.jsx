...
function Favorite() {
  ...
  const handleSave = (e) => {
    e.stopPropagation()
    dispatch({type: "change/favorite", payload: {request: request, videosCount: rangeValue}})
  }
      ...
      <div className='content'>
          ...
          {titles && titles.map((item, index) => {
            ...
            <div className='one__title'>
              <NavLink key={index} to="/main">
                <div className='content__one__title' onClick={() => handleSearch(item.favorite, item.videosCount)}>
                  {item.favorite}
                </div>
              </NavLink>
              <div className='content__change' onClick={() => handleChange(index)}></div>
        ...
        <Modal modal={modal} setModal={setModal}>
          ...
          <div className='request'>
            <h5>Запрос</h5>
            <input className='request__input' value={request} onChange={(e) => setRequest(e.target.value)} 
            type="text" onClick={e => e.stopPropagation()}/>
            <h5>Название</h5>
            <input className='request__input' value={name} onChange={(e) => setName(e.target.value)} 
            type="text" onClick={e => e.stopPropagation()}/>
            <h5>Сортировать по</h5>
            <input className='request__input' value={sort} onChange={(e) => setSort(e.target.value)} 
            type="text" onClick={e => e.stopPropagation()}/>
            <h5>Максимальное кол-во</h5>
            <div className='range__flex'>
              <input className='range' type="range" 
              value={rangeValue} onChange={(e) => setRangeValue(e.target.value)} 
              id="range" min="0" max="50" onClick={e => e.stopPropagation()}/>
              <div className='range__count'>{rangeValue}</div>
            </div>
            <button className='non__save'>Не сохранять</button>
            <button className='save' onClick={(e) => handleSave(e)}>Сохранить</button>
          </div>
        </Modal>
